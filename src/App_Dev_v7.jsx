...
________________________________________#POV #Wealth #Billionaire #MoneyPsychology #FinanceExpl #Success`); // New Field
...
The door is about to open. It always does.`);
  ...
  const analyzeAndGenerate = async () => {
    ...
    try {
      ...
      TASK 1: Write a highly engaging, original production script for a video titled "${videoTitle}". Use the provided transcript strictly as an inspiration for the theme, tone, and core message, but DO NOT copy it line-for-line. Rewrite the narrative completely with fresh phrasing, creative pacing, and strong hooks to make it a unique 8-minute video.
  ...
  const generateSingleImage = async (index) => {
    ...
    try {
      const url = await callImagen(imagePrompts[index].prompt);
      ...
      <main className="max-w-5xl mx-auto bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden min-h-[600px]">
        ...
        {step === 4 && (
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 gap-6 max-h-[700px] overflow-y-auto pr-4">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {typeof scene === 'object' && scene.script && (
                      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col">
                         ...
                         <span className="text-base text-slate-800 font-serif leading-relaxed line-clamp-none whitespace-pre-wrap italic overflow-y-auto max-h-[200px]">"{scene.script}"</span>
                      </div>
                    )}
                    
                    <div className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap font-mono bg-indigo-50/50 p-4 rounded-xl border border-indigo-100/50 h-full overflow-y-auto max-h-[300px]">
        ...
        {step === 5 && (
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-h-[750px] overflow-y-auto pr-4 pb-12">
              {imagePrompts.map((scene, i) => (
                <div key={i} className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden flex flex-col shadow-sm">
